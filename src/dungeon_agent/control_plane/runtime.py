...
def _client(service: str, **kwargs: Any) -> Any:
    return _boto3().client(service, config=kwargs.pop("config", _CONFIG), **kwargs)
...
_BEDROCK_CONFIG = _aws_config(
    retries={"total_max_attempts": 2, "mode": "adaptive"}, connect_timeout=3, read_timeout=120
)
...
def _management_client() -> Any:
    return (
        None
        if (endpoint := os.environ.get("WS_MANAGEMENT_ENDPOINT")) is None
        else _client("apigatewaymanagementapi", endpoint_url=endpoint)
    )
...
def _build_delivery() -> BestEffortEventDelivery | None:
    return (
        None
        if (client := _management_client()) is None
        else BestEffortEventDelivery(_connection_repository(), client)
    )


class _ApiGatewaySender:
    def __init__(self, client: Any, connections: DynamoDbConnectionRepository) -> None:
        self._client, self._connections = client, connections
...
def _microvm_manager() -> LambdaMicrovmManager:
    return LambdaMicrovmManager(
        _client("lambda-microvms"), os.environ["MICROVM_IMAGE_NAME"], _REGION
    )
...
def _build_speech_handlers() -> SpeechHttpHandlers | None:
    ...
    s3 = _client("s3", region_name=_REGION, endpoint_url=f"https://s3.{_REGION}.amazonaws.com")
...
def _build_websocket_adapter(endpoint: str) -> ApiGatewayWebSocketAdapter:
    ...
    service = RealtimeSessionService(connections, _REPOSITORY, campaign_store=_CAMPAIGN_REPOSITORY)
