...
from dungeon_agent.plane_shared.domain.enums import CampaignStatus, ErrorCode
...
# Lab taxonomy: same HTTP API deploy; routes belong to one conceptual plane.
ROUTE_PLANE: dict[str, Literal["control", "data"]] = {
    ...
    "GET /sessions": "control",
    "GET /sessions/{sessionId}": "control",
    "POST /sessions/{sessionId}/abandon": "control",
    "POST /sessions/{sessionId}/actions": "data",
    "GET /sessions/{sessionId}/events": "data",
    "POST /speech": "data",
}
...
class ApiGatewayHttpAdapter:
    def __init__(
        ...
        *,
        ...
        def submit_action(
            e: Mapping[str, Any], h: Mapping[str, str], i: AuthenticatedIdentity, c: str
        ) -> HttpResult:
            return self._require_actions().submit_action(
                i,
                _session_id(e),
                SubmitActionRequest.model_validate(_json_body(e)),
                idempotency_key=h.get("idempotency-key", ""),
                correlation_id=c,
            )

        def list_session_events(
            e: Mapping[str, Any], _h: Mapping[str, str], i: AuthenticatedIdentity, c: str
        ) -> HttpResult:
            return self._require_actions().list_events(
                i, _session_id(e), after=_replay_after(e), correlation_id=c
            )

        self._routes: dict[str, RouteHandler] = {
            "POST /sessions": lambda e, h, i, c: sessions.create_session(
                i,
                CreateSessionRequest.model_validate(_json_body(e)),
                idempotency_key=h.get("idempotency-key", ""),
                correlation_id=c,
            ),
            "POST /sessions/{sessionId}/actions": submit_action,
            "GET /sessions/{sessionId}": lambda e, _h, i, c: sessions.get_session(
                i, _session_id(e), correlation_id=c
            ),
            "GET /sessions/{sessionId}/events": list_session_events,
            "GET /sessions": lambda e, _h, i, c: _list_active_sessions(sessions, e, i, c),
            "POST /sessions/{sessionId}/abandon": lambda e, _h, i, c: sessions.abandon_session(
                i, _session_id(e), correlation_id=c
            ),
            "POST /campaigns": lambda e, h, i, c: campaigns.create_campaign(
                i,
                CreateCampaignRequest.model_validate(_json_body(e)),
                idempotency_key=h.get("idempotency-key", ""),
                correlation_id=c,
            ),
    ...
    def _require_actions(self) -> ActionHttpHandlers:
        if self._actions is None:
            raise RuntimeError("data-plane action handlers are not configured")
...
def _normalized_headers(value: object) -> dict[str, str]:
    return (
        {str(key).lower(): str(header) for key, header in value.items()}
